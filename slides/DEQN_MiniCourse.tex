...
% --- NAS Notebook Reference ---
\begin{frame}{NAS Hands-on Notebook}
\textbf{Hands-on:} \texttt{05\_Neural\_Architecture\_Search.ipynb}

\vspace{0.5em}
\begin{enumerate}
    \item Define a test function (Genz Gaussian on $[0,1]^2$)
    \item Specify a hyperparameter search space:
    \begin{itemize}
        \item Depth (1--5 layers), width (32--256), activation (ReLU, tanh, ELU, Swish)
        \item Dropout, batch normalization, optimizer, learning rate, batch size
    \end{itemize}
    \item Run \textbf{30 random trials} with early stopping
    \item Select \textbf{top-5 architectures}, retrain for longer evaluation
    \item Analyse which hyperparameters matter most
\end{enumerate}

\vspace{0.5em}
{\small Runtime: $\sim$2--3 minutes on CPU. All code in the \texttt{code/} directory.}
\end{frame}
...
\textbf{Total loss:} $\ell = \sum_{k=1}^K \ell_k$.\quad The gradient: $\nabla_\rho \ell = \sum_{k=1}^K \nabla_\rho \ell_k$.

\vspace{0.3em}
When $\ell_j \gg \ell_k$ for some $j$:
\begin{itemize}\itemsep1pt
    \item $\|\nabla_\rho \ell_j\| \gg \|\nabla_\rho \ell_k\|$~~$\Rightarrow$~~update direction dominated by component $j$
    \item Components $k \neq j$ may even \emphc{increase} during training
    \item Network satisfies \emph{one} equation well but violates the others
\end{itemize}
...
\begin{itemize}\itemsep1pt
    \item All components contribute equally
    \item Network balances all conditions
...
\midrule
Equal weights & $w_k = 1/K$ & Ignores scale differences \\[1pt]
Manual tuning & Hand-pick $w_k$ & Does not adapt during training \\[1pt]
Inverse-loss & $w_k \propto 1/\ell_k$ & Oscillates on noisy components \\[1pt]
GradNorm (Chen+ 2018) & Match gradient norms across tasks & Per-task gradient computation; expensive \\[1pt]
Uncertainty wt.\ (Kendall+ 2018) & Learn $\sigma_k$: $w_k \!\propto\! 1/(2\sigma_k^2)$ & Extra parameters; can collapse \\[1pt]
\emphc{ReLoBRaLo} (Bischof+ 2021) & Relative ratios + random lookback + smoothing & \emphc{Lightweight; robust; no extra params to learn} \\
\bottomrule
\end{tabular}
}
\end{center}
...
\vspace{0.3em}
{\small \textbf{ReLoBRaLo's appeal:} no gradient-norm computation, no extra learnable parameters, strong empirical performance across PINNs, DEQNs, and multi-task regression.}
\end{frame}
...
% --- Look-up Tables to Neural Networks ---
...
\begin{frame}{Pseudo-States: The Key Innovation}
...
% --- Why Surrogates Are Different ---
\begin{frame}{Why Surrogates $\neq$ Standard ML}
\begin{columns}[T]
...
% --- Application: Structural Estimation ---
\begin{frame}{Application I: Structural Estimation}
\textbf{Goal:} Estimate structural parameters $\theta$ from data via GMM:
\[
\hat{\theta}_{GMM} = \argmin_\theta\; g_n(\theta)^\top \, W \, g_n(\theta),
\]
where $g_n(\theta) = \frac{1}{n}\sum_{i=1}^n m(y_i, \theta)$ and computing $m(\cdot)$ requires solving the model.

\vspace{0.3cm}
\begin{block}{Surrogate-based estimation}
\begin{enumerate}
  \item Replace $f(\cdot)$ with surrogate $\phi(\cdot\,|\,\theta_{NN})$.
  \item \emphc{Automatic differentiation} gives analytical gradients $\nabla_\theta g_n$.
  \item First-order conditions become a \emph{system of equations} --- solve in milliseconds.
\end{enumerate}
\end{block}

\vspace{0.3cm}
{\small \textbf{Result:} Full GMM estimation in \emphc{seconds} instead of weeks.\\
...
\node[mlstep, right=1.5cm of model, fill=softblue!30] (surr) {Deep Surrogate};
\node[mlstep, right=1.5cm of surr, fill=softgreen!30] (bayes) {Bayesian Inf.};
\node[mlstep, right=1.5cm of bayes, fill=softorange!30] (post) {$p(\theta|\text{data})$};

\draw[->, thick] (model) -- node[above=2pt, font=\scriptsize]{train} (surr);
\draw[->, thick] (surr) -- node[above=2pt, font=\scriptsize]{$10^6$ evals} (bayes);
...
\vspace{0.2cm}
\begin{itemize}
  \item Surrogate over \emphc{100+ climate \& economic parameters}
  \item Posterior distributions of \emphc{damage functions} and \emphc{optimal carbon tax}
  \item MCMC requires millions of model evaluations $\Rightarrow$ only feasible with surrogate
\end{itemize}
\end{frame}

% --- Implied Volatility Surface ---
\begin{frame}{Application III: The Implied Volatility Surface}
...
\begin{tikzpicture}[
    ...
    hidden/.style={rectangle, draw=darkred, fill=red!5, minimum width=1.5cm, minimum height=2cm, rounded corners},
    ...
    \node[input] (P) at (0, -1.2) {$\theta_{\text{mod}}$};
    \node[hidden] (net) at (3, 0) {\textbf{Surrogate}};
    \node[output] (IV) at (6, 0) {$\sigma_{IV}$};
    \draw[arr] (K) -- (net);
    \draw[arr] (S) -- (net);
    \draw[arr] (P) -- (net);
    \draw[arr] (net) -- (IV);
...
\begin{itemize}\itemsep1pt
    ...
    \item \textbf{Arbitrage-free:} Enforce constraints (no calendar/butterfly arb) in the loss.
    \item \textbf{Hands-on:} See \texttt{06\_Surrogate\_Primer.ipynb} for a Black--Scholes example.
\end{itemize}
\end{frame}
...
% --- Gaussian Processes ---
\begin{frame}{Gaussian Processes: A Complementary Tool}
\begin{block}{Definition}
A \emphc{Gaussian Process} is a distribution over functions:
...
\begin{itemize}\itemsep1pt
  \item \emphc{Nonparametric:} model grows with data
  \item \emphc{Built-in UQ:} posterior mean \emph{and} variance
  \item \emphc{Principled kernels:} encode smoothness, scale
  \item \emphc{Hyperparameter learning:} marginal likelihood
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Posterior (noiseless):}
\begin{align*}
\bar{f}_* &= \bm{k}_*^\top K^{-1} \bm{f} \\
\sigma_*^2 &= k(\x_*, \x_*) - \bm{k}_*^\top K^{-1} \bm{k}_*
\end{align*}
\vspace{-0.3cm}

{\small Near data: \emphc{small variance}.\\
Far from data: \emphc{large variance} (reverts to prior).}
\end{column}
\end{columns}

\vspace{0.3em}
\begin{alertblock}{Complementary to DNNs}
GPs: best for $d \leq 10$ with built-in UQ. \quad DNNs: best for $d \gg 10$ with large data.
\end{alertblock}
\end{frame}
...
\midrule
Uncertainty quantification & \textcolor{darkgreen}{Built-in (posterior var.)} & \textcolor{darkred}{Requires ensembles} \\
Hyperparameter tuning & \textcolor{darkgreen}{Marginal likelihood} & \textcolor{softorange}{Grid / Bayesian opt.} \\
...
\begin{itemize}
  \item \textbf{$d \leq 10$, need UQ:} Use GPs (+ BAL for efficient sampling).
  \item \textbf{$d > 10$, large data:} Use DNNs (deep surrogates).
\end{itemize}
\end{alertblock}
\end{frame}
...
\vspace{-4pt}
\[
U(\x) = \rho \cdot \mu(\x) + \frac{\beta}{2}\log\sigma^2(\x)
\]
\vspace{-8pt}
\begin{itemize}\setlength{\itemsep}{1pt}
  \item $\mu(\x)$: posterior mean (\emphc{exploitation})
  \item $\sigma^2(\x)$: posterior variance (\emphc{exploration})
  \item Pure exploration ($\rho = 0$): select where uncertainty is highest
\end{itemize}
\end{block}
...
\node[mlstep, fill=softorange!30] (init) {Design $\mathcal{D}_0$};
\node[mlstep, right=0.6cm of init, fill=softblue!30] (gp) {Fit GP};
\node[mlstep, right=0.6cm of gp, fill=softgreen!20] (score) {Score $U(\x)$};
\node[mlstep, right=0.6cm of score] (select) {Select $\x_{\text{new}}$};
\node[mlstep, right=0.6cm of select] (query) {Query $f(\x_{\text{new}})$};
\draw[->, thick] (init) -- (gp);
\draw[->, thick] (gp) -- (score);
\draw[->, thick] (score) -- (select);
\draw[->, thick] (select) -- (query);
\draw[->, thick, rounded corners=5pt] (query.south) -- ++(0,-0.4) -| (gp.south);
\end{tikzpicture}
\end{center}
\end{frame}

% --- BAL vs Uniform ---
\begin{frame}{BAL vs Uniform Sampling}
{\small Target: $f(x) = |\sin(\pi x/2)|$ (has kinks at $x \in \{0, \pm 2\}$). Same budget: 11 points.}
...
\begin{axis}[
    ...
    title={\small \textbf{BAL}: concentrates near kinks},
    xlabel={$x$}, ylabel={$f(x)$},
    xlabel style={font=\small}, ylabel style={font=\small},
    tick label style={font=\scriptsize},
    xmin=-1, xmax=1, ymin=-0.2, ymax=1.2,
    grid=major, grid style={gray!20},
    every axis plot/.append style={no markers},
]
\addplot[uzhblue, thick, domain=-1:1, samples=200] {abs(sin(3.14159*x/2 r))};
\addplot[only marks, mark=*, mark size=2.5pt, harvardcrimson] coordinates
    {(-0.95,0.81) (-0.6,0.81) (-0.35,0.57) (-0.15,0.23) (-0.05,0.08)
     (0.0,0.0) (0.05,0.08) (0.15,0.23) (0.35,0.57) (0.6,0.81) (0.95,0.81)};
\end{axis}

% Right: Uniform
\begin{axis}[
    at={(bal.east)}, anchor=west, xshift=1cm,
    width=7cm, height=5cm,
    title={\small \textbf{Uniform}: wastes points in smooth regions},
    xlabel={$x$},
    xlabel style={font=\small},
    tick label style={font=\scriptsize},
    xmin=-1, xmax=1, ymin=-0.2, ymax=1.2,
...
\addplot[uzhblue, thick, domain=-1:1, samples=200] {abs(sin(3.14159*x/2 r))};
\addplot[only marks, mark=*, mark size=2.5pt, softorange] coordinates
    {(-1.0,0.0) (-0.8,0.95) (-0.6,0.81) (-0.4,0.59) (-0.2,0.31)
     (0.0,0.0) (0.2,0.31) (0.4,0.59) (0.6,0.81) (0.8,0.95) (1.0,0.0)};
\end{axis}
\end{tikzpicture}
\end{center}

\vspace{0.1cm}
{\small \textcolor{harvardcrimson}{\textbf{BAL}} clusters points near $x=0$ (kink) where the function is hardest to approximate.
\textcolor{softorange}{\textbf{Uniform}} spreads evenly --- several points wasted in smooth regions ($|x| > 0.5$).}
\end{frame}

% --- Summary: Deep Surrogates ---
...
\begin{enumerate}
  ...
  \item \textbf{GPs + BAL:} complementary for moderate $d$ with built-in UQ.
\end{enumerate}

\vspace{0.3cm}
\begin{block}{Hands-on notebooks}
\texttt{06\_Surrogate\_Primer.ipynb} --- Black--Scholes surrogate with IV inversion.\\
\texttt{07\_GP\_and\_BAL.ipynb} --- GP regression from scratch and Bayesian Active Learning.
\end{block}
\end{frame}


% ============================================================================
% PART VII: HANDS-ON EXERCISES
% ============================================================================
...
\texttt{05\_Neural\_Architecture\_Search} & NAS via random search: hyperparameter space, top-$k$ selection \\
\texttt{06\_Surrogate\_Primer} & Deep surrogates: Black--Scholes pricing \& implied volatility inversion \\
\texttt{07\_GP\_and\_BAL} & GP regression from scratch, scikit-learn, Bayesian Active Learning \\
\bottomrule
\end{tabular}
}
\end{center}
